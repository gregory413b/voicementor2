    ...
    const fetchData = async () => {
      ...
      try {
        const { data: messagesData, error: messagesError } = await supabase
          ...
          `)
          .or(`conversation.client_id.eq.${user.id},conversation.mentor_id.eq.${user.id}`)
        ...
        const processedMessages = messagesData
          ...
          .map(msg => ({
            ...msg,
            unread_count: Math.floor(Math.random() * 4), // Placeholder for unread count
        ...
        const latestMessages = processedMessages.reduce((acc, message) => {
          const existingIndex = acc.findIndex(m => m.conversation.id === message.conversation.id);
          if (existingIndex === -1) {
            acc.push(message);
          }
          return acc;
